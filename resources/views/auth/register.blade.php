        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mt-4">
                ...
                <select class="block mt-1 mr-1 rounded-md border-gray-300" id="region" name='region' :value="old('region')">
                    <option disabled="disabled" selected="selected">CC </option>
                    <?php
                    foreach ($arrRegion as $region) {
                        $selected = $region === 'PH' ? 'selected' : '';
                        echo "<option value='$region' $selected>+" . $phoneUtil->getCountryCodeForRegion($region) . "</option>";
                    }
                    ?>
                </select>
                <x-input id="contact" class="block mt-1 w-full" type="number" name="contact" :value="old('contact')" />
                </div>
            </div>
